...
def Album_str(A: Album) -> str:
    ...
    for obj in A.songs:
        empty_str += Song_str(obj) + "\n"
    return empty_str
print(Album_str(Album1))
print()
print()
print()

def Songdisplay_str(s_disp: Songdisplay) -> str:
    """ Makes s_disp into a str"""
    empty_str = ''
    for obj in s_disp:
        empty_str += "Artist: {:18s}  Album's title: {:25s}  year: {:4d}  Track: {:2d}  Song's title: {:30s}  Length: {:3d}  Play Count: {:2d}".format(obj.artist, obj.a_title, obj.year, obj.track, obj.s_title, obj.length, obj.play_count)+'\n'
    return empty_str
obj = top_n_played(MUSIC, 10)
print(Songdisplay_str(obj))

print()
print("--- e2 ---")
print()

def ALBUM_TRACKS(A: Album):
    return len(A.songs)
...
def unplayed_songs(M_col: "List of Albums") -> list:
    """ Takes list of albums and returns list"""
    new_list = []
    for obj in M_col:
        A_to_song = Album_to_Songdisplays(obj)
        for obj1 in A_to_song:
            if obj1.play_count == 0:
                new_list.append(obj1)
    return new_list
Song_unplayed = unplayed_songs(MUSIC)
print(Songdisplay_str(Song_unplayed))
...
def times_album(A: Album):
    count = 0
    for obj in A.songs:
        count += obj.length*obj.play_count
    return count
def favorite_album(MC: 'List of albums'):
    MC.sort(key = times_album)
    return MC[-1]
...
def top_n(M_col: 'List of Albums', num: int, keyfunction: 'Function on Albums', Boolean: 'True or Flase'):
    if Boolean == True:
        list_Songdisplays = all_Songdisplays(M_col)
        list_Songdisplays.sort(key=keyfunction, reverse=True)
        return list_Songdisplays[:num]
    else:
        list_Songdisplays = all_Songdisplays(M_col)
        list_Songdisplays.sort(key=keyfunction)
        return list_Songdisplays[:num]
...
def favorite_album2(M_col: 'List of Albums', fun: "Favorite measurement function"):
    M_col.sort(key = fun)
    return M_col[-1]
print(Album_str(favorite_album2(MUSIC, times_album)))
print()
...
print("--- e8 ---")
print()
def collection_search(M_col: 'List of Albums', key: 'key word'):
    new_list = []
    list_of_Songdisplays = all_Songdisplays(M_col)
    for obj in list_of_Songdisplays:
        if key in obj.artist:
            new_list.append(obj)
        elif key in obj.a_title:
            new_list.append(obj)
        elif key in obj.s_title:
            new_list.append(obj)
    return new_list
print(collection_search(MUSIC, 'The Drop'))
